...
// THE signUp Flow (Step 1 -> 2 -> 3)
type Step = 1 | 2 | 3;

export default function SignUpModal({ isOpen, onClose, mode }: SignUpModalProps) {
  ...
  const handleNext = async () => {
    // If Login Mode, go to Success/close (Simulation)
    if (mode === 'login') {
      setLoading(true);
    ...
    // If Sign Up, go through the Wizard steps
    if (step === 2) {
      setLoading(true);
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between items-center mb-8">
          {/* Progress Indicators in Sign Up Mode */}
          {mode === 'signup' ? (
            ...
            <div /> // Empty div to keep the close button aligned right
          )}
          <button
            onClick={onClose}
        ...
        <div className="relative min-h-[340px]">
          <AnimatePresence mode="wait" custom={step}>
            ...
            {/* STEP 1: LOGIN OR SIGNUP CREDENTIALS */}
            {step === 1 && (
              <motion.div
                key="step1"
              ...
              >
                <h2 id="modal-title" className="text-2xl font-bold text-green-darker mb-2">
                  {mode === 'login' ? 'Welcome Back' : 'Create account'}
                </h2>
                <p className="text-gray-500 mb-6">
                  {mode === 'login' ? 'Enter your details to access your account.' : 'Start building your financial future in the UK.'}
                </p>

                <div className="flex flex-col gap-4 flex-1">
                  {mode === 'signup' && (
                    <Input placeholder="Full Name" startIcon={<User className="w-4 h-4 text-gray-400" />} />)}
                  <Input
                ...
                <Button onClick={handleNext} width="full" className="mt-6" loading={loading}>
                  {mode === 'login' ? 'Login' : 'Continue'}
                  {mode === 'signup' && <ArrowRight className="w-4 h-4 ml-2" />}
                </Button>

                {/* Helper Link */}
                {mode === 'login' && (
                    <p className="text-center text-sm text-gray-500 mt-4">
                        Don&apos;t have an account? <span className="text-green-normal cursor-pointer hover:underline" onClick={onClose}>Sign up</span>
                    </p>
                )}
              </motion.div>
            )}

            {/* STEP 2: ROLE SELECTION (in sign Up mode)*/}
            {step === 2 && (
              <motion.div
                key="step2"
                variants={slideVariants}
            ...
            {/* STEP 3: SUCCESS (in Sign Up mode) */}
            {step === 3 && (
              <motion.div
                key="step3"
                variants={slideVariants}
...
function RoleOption({ icon, label, selected, onClick }: { icon: ReactNode, label: string, selected: boolean, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
